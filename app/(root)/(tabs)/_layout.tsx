...
const TabIcon = ({ focused, source }: { source: ImageSourcePropType, focused: boolean }) => {
    return (
        <View
            className={`flex flex-row justify-center items-center w-12 h-12 rounded-full mb-8 ${focused ? "" : ""} `}
        //I have to give harcoded margin bottom because the icons are not centered with flex, justify center and iteams center
        >
            <View
                className={`rounded-full flex items-center p-2 w-12 h-12 justify-center ${focused ? "bg-general-400" : ""}`}
            >
                <Image
                    source={source}
                    tintColor={"white"}
